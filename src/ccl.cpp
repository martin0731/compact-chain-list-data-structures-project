...
int CompactChainList::countSubsequencesFrom(vector<Element> &expanded,
                                            vector<Element> &subsequence,
                                            int posExpanded, int posSubseq) {
  int ans = 0;
    ...
    if (posExpanded >= expanded.size()) {
      ...
      for (int i = posExpanded; i < expanded.size(); i++) {
        if (expanded[i] == subsequence[posSubseq])
          ans += countSubsequencesFrom(expanded, subsequence, i + 1,
                                       posSubseq + 1);
      }
    }
  }
...
void CompactChainList::appendRemainingBlocks(Node *current, Node *startNode,
...
CompactChainList::CompactChainList(const CompactChainList &other) {
  ...
  if (other.first != NULL) {
    ...
    do {
      Node *newNode =
          new Node(current->getVal(), current->getLength(), NULL, NULL);
      ...
      if (node != NULL) {
        if (node->getVal() == e) {
          ...
          if (localPos == 0) {
            ...
            Node *rightNode =
                new Node(originalVal, rightLength, nextNode, newNode);
...
int CompactChainList::size() { // Complejidad constante, listSize guarda cuántos
                               // elementos totales hay en la lista
  return listSize;
}
...
int CompactChainList::getIndexFirstConsecutiveOcurrence(
    vector<Element> &subsequence) {
  int index = -1;
...
bool CompactChainList::operator<(const CompactChainList &other) const {
  bool result = false;
