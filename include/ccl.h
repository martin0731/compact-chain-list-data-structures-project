...
private:
    ...
    // función auxiliar para agregar un bloque, utilizada en constructor con vector 
    void insertBlock(Element v, int count);
    
    // función auxiliar para agregar los bloques restantes en la fusión lexicográfica
    static void appendRemainingBlocks(Node* current, Node* startNode, CompactChainList& result);

    // función auxiliar para encontrar el nodo y la posición local correspondiente a la p global, para operación de set e insertElement
    Node* findNodeAtPosition(int position, int& localPos);

    // función auxiliar para contar el número de bloques para el print CCL
    int blockCount();

    // función auxiliar para contar subsecuencias de forma recursiva en getOcurrences
    int countSubsequencesFrom(vector<Element>& expanded, vector<Element>& subsequence,int posExpanded, int posSubseq);
...
public:
    ...
    bool operator<(CompactChainList& other);                                                  
    bool operator==(CompactChainList& other);                                                 
